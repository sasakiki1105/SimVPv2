...
def get_display_range(true, scale_mode="true_p1p99"):
    """
    Determine common vmin/vmax for Input/True/Pred based on TRUE only.

    scale_mode:
      - true_p1p99 : use 1st and 99th percentile of true
      - true_minmax: use min/max of true
    """
    true = np.asarray(true)

    if scale_mode == "true_p1p99":
    ...
    # fallback for degenerate / flat cases
...
def save_one(fig_path, in_last, true, pred, title, cmap="viridis", scale_mode="true_p1p99"):
    err = np.abs(pred - true)

    # Common scale for Input/True/Pred based on TRUE only
    vmin, vmax = get_display_range(true, scale_mode=scale_mode)

    # Error scale per image
    evmin = float(np.min(err))
    evmax = float(np.max(err))
    if evmax <= evmin:
        evmax = evmin + 1e-6
...
def main():
    ...
    ap.add_argument("--scale_mode", default="true_p1p99",
                    choices=["true_p1p99", "true_minmax"],
                    help="common display range for Input/True/Pred, determined from True only")
    args = ap.parse_args()
    ...
    def cname(c):
        return names[c] if 0 <= c < len(names) else f"ch{c}"

    print("inputs:", X.shape, X.dtype)
    print("preds: ", P.shape, P.dtype)
    print("trues: ", Y.shape, Y.dtype)
    print(f"export N={N}/{N_all}, Tout={Tout}/{Tout_all}, channels={ch_list}, scale_mode={args.scale_mode}")
