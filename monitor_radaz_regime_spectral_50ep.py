#!/usr/bin/env python3
"""Report command-line progress for the mixed RadAz spectral queue."""
...
def process_state() -> tuple[int | None, bool]:
...
def epoch_records(workdir: Path) -> list[tuple]:
...
def status(ex_name: str, queue_running: bool) -> str:
    ...
    if model_param.exists():
        updated = datetime.fromtimestamp(model_param.stat().st_mtime)
...
def main() -> None:
    ...
    print("=" * 116)
    print(f"RadAz mixed spectral monitor  {datetime.now():%Y-%m-%d %H:%M:%S}")
    ...
    for key, ex_name in CASES:
        print(f"{key:<26} {status(ex_name, running)}")
